...
(function(){
    function Album($elem) {
        ...
        this.createImage = function(image_src){
            ...
            cover.innerHTML = '<span class="icon-cancel-2"/>';
        ...
        var that = this;
        $elem.find('[upload]').on('upload:success', function(data){
            var result = JSON.parse(data.jqXHR.responseText);
            that.insertImage(result.url);
            that.setProgress('over');
        });

        $elem.delegate('.simple-text-content [upload]', 'upload:error', function(e, data){
            that.setProgress('over');
        });
        $elem.delegate('.simple-text-content [upload]', 'upload:progress', function(_, data){
